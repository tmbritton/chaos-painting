...
const eventBus = (() => {
   ...
   */
  const registerEntity = (id: string, receiveMessage: MessageReceptor) => entityMap[id] = receiveMessage
   ...
   */
  const send = (senderId: string, recipientId: string, message: Message) => entityMap[recipientId](senderId, message)
